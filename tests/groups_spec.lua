...
describe("Heap groups", function()
	...
	it("styles Mason without depending on Lazy", function()
		config.setup({ auto = false, plugins = { lazy = false, mason = true } })
		vim.cmd("colorscheme heap")

		local mason_normal = vim.api.nvim_get_hl(0, { name = "MasonNormal", link = false })
		local mason_header = vim.api.nvim_get_hl(0, { name = "MasonHeader", link = false })
		assert.is_truthy(mason_normal.fg)
		assert.is_truthy(mason_normal.bg)
		assert.is_truthy(mason_header.fg)
		assert.is_nil(mason_normal.link)
		assert.is_nil(mason_header.link)
	end)
